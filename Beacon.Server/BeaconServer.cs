using Beacon.API;
using Beacon.API.Events;
using Beacon.API.Models;
using Beacon.Server.Config;
using Beacon.Server.Net;
using Beacon.Server.Plugins;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
using Microsoft.Extensions.Logging;
using System.Diagnostics;
using System.Net;
using System.Net.Sockets;
using System.Text.Json;

namespace Beacon.Server
{
    internal class BeaconServer : BackgroundService, IServer
    {
        public IMinecraftEventBus EventBus { get; }
        public ILogger<IServer> Logger { get; }
        public ServerConfiguration Configuration { get; private set; }

        private const string SERVER_ICON = "data:image/png;base64,";
        private readonly IServiceProvider _provider;
        private readonly IPluginController _pluginController;

        public BeaconServer(
            ILogger<BeaconServer> logger,
            IServiceProvider provider,
            IPluginController plugins,
            IMinecraftEventBus eventBus)
        {
            Logger = logger;
            Configuration = ServerConfiguration.Default;
            _provider = provider;
            _pluginController = plugins;
            EventBus = eventBus;
        }
        public Version Version => GetType().Assembly.GetName().Version ?? new();
        public async ValueTask ReloadAsync()
        {
            using var scope = this.Logger.BeginScope("ServerReload");
            Logger.LogInformation("Reloading server");

            await _pluginController.UnloadAsync();
            await _pluginController.LoadAsync();
        }
        public ValueTask<ServerStatus> GetStatusAsync()
        {
            return ValueTask.FromResult(new ServerStatus()
            {
                Version = new ServerVersionModel
                {
                    Name = "1.18.1",
                    Protocol = 757
                },
                Players = new OnlinePlayersModel
                {
                    Max = 9,
                    Online = 6,
                    Sample = new OnlinePlayerModel[]
                    {
                        new OnlinePlayerModel
                        {
                            Name = "Gottem",
                            Id = new Guid().ToString(),
                        }
                    }
                },
                Description = new DescriptionModel
                {
                    Text = "A server running on Beacon!"
                },
                Favicon = SERVER_ICON
            });
        }

        protected override async Task ExecuteAsync(CancellationToken cToken)
        {
#if DEBUG
            Logger.LogWarning("Running in debug mode! Performance will be impaired");
#else
            _logger.LogInformation("Running in production mode");
#endif
            Logger.LogDebug("Debug logs are enabled for this log sink");

            await LoadConfigurationAsync();
            await InitializeAsync();

            await Task.WhenAny(
                AccecptConnectionsAsync(cToken),
                AcceptCommandLineInput(cToken)
            );
        }

        private async Task InitializeAsync()
        {
            var watch = Stopwatch.StartNew();

            // Plugins
            await _pluginController.LoadAsync();

            watch.Stop();
            Logger.LogInformation("Server booted in {seconds} seconds", watch.ElapsedMilliseconds / 1000.0);
        }
        private async Task LoadConfigurationAsync()
        {
            Logger.LogInformation("Reading configuration");
            var file = new FileInfo("server_settings.json");

            if (!file.Exists)
            {
                using var stream = file.OpenWrite();
                await JsonSerializer.SerializeAsync(stream, Configuration);
                return;
            }

            try
            {
                using var stream = file.OpenRead();
                var parsedConfig = await JsonSerializer.DeserializeAsync<ServerConfiguration>(stream);
                if (parsedConfig != null)
                {
                    Configuration = parsedConfig;
                    return;
                }
                Logger.LogWarning("Could not read server configuration. Resetting to the default one");

            }
            catch (Exception e)
            {
                Logger.LogWarning(e, "Could not read server configuration. Resetting to the default one");
            }
            file.Delete();
            await JsonSerializer.SerializeAsync(file.OpenWrite(), Configuration);

        }
        private Task AcceptCommandLineInput(CancellationToken cToken)
        {
            while (!cToken.IsCancellationRequested)
            {
                Console.ReadLine();
            }
            return Task.CompletedTask;
        }
        private async Task AccecptConnectionsAsync(CancellationToken cToken)
        {
            Logger.LogInformation("Start listening for clients");

            var listener = new TcpListener(IPAddress.Any, 25565);
            listener.Start();

            while (!cToken.IsCancellationRequested)
            {

                var client = await listener.AcceptTcpClientAsync(cToken);
                _ = HandleTcpClient(client, cToken);
            }
        }
        private async Task HandleTcpClient(TcpClient tcp, CancellationToken cToken)
        {
            var connection = new BeaconConnection(this, tcp);

            // Fire event to notify plugins a TCP connection has been made.
            var e = new TcpConnectedEvent(this, connection);
            await this.EventBus.FireEventAsync(e, cToken);

            if (e.IsCancelled)
            {
                connection.Dispose();
                return;
            }

            var faulted = false;

            try
            {
                await connection.AcceptPacketsAsync(cToken);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "An exception occured while a connection was accepting packets");
                faulted = true;
            }
            finally
            {
                connection.Dispose();
            }

            // Fire event to notify plugins a TCP connection has been lost.
            await this.EventBus.FireEventAsync(new TcpDisconnectedEvent(this, faulted,  connection.RemoteAddress), cToken);
        }
    }
}
