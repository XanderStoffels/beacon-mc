...
namespace Beacon.Server
{
    internal class BeaconServer : BackgroundService, IServer
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken cancelToken)
        {
            ...
            await AccecptConnectionsAsync(cancelToken);
        }

        private async ValueTask<ServerConfiguration> LoadConfiguration(CancellationToken cToken = default)
        {
            ...
            if (file.Exists)
            {
                try
                {
                    var parsedConfig = await JsonSerializer.DeserializeAsync<ServerConfiguration>(file.OpenRead(), cancellationToken: cToken);
