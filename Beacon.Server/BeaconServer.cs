...
namespace Beacon.Server
{
    internal class BeaconServer : BackgroundService, IServer
    {
        ...
        public ServerConfiguration Configuration { get; protected set; }

        public BeaconServer(ILogger<BeaconServer> logger,
        ...
        private async Task AccecptConnectionsAsync(CancellationToken cToken)
        {
            ...
            while (!cToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    _ = con.AcceptPacketsAsync(cToken);
                }
                catch (Exception e)
                {
                    _logger.LogWarning(e, "Lost connection to a client");
                }
