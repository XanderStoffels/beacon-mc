/*using System.Diagnostics;
using System.Net;
using System.Net.Sockets;
using System.Reflection;
using System.Text.Json;
using Beacon.API;
using Beacon.API.Commands;
using Beacon.API.Events;
using Beacon.API.Models;
using Beacon.Server.Attributes;
using Beacon.Server.CLI;
using Beacon.Server.Commands;
using Beacon.Server.Commands.Builtins;
using Beacon.Server.Config;
using Beacon.Server.Net;
using Beacon.Server.Plugins;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
using Microsoft.Extensions.Logging;

namespace Beacon.Server;

public class OldBeaconServer : BackgroundService, IServer
{
    private const string ServerIcon =
        "data:image/png;base64,";

    private readonly ILogger<IServer> _logger;
    private readonly PluginManager _pluginManager;
    private ServerConfiguration _config;

    public OldBeaconServer(
        ILogger<OldBeaconServer> logger,
        IServiceProvider provider)
    {
        _logger = logger;
        _pluginManager = provider.GetRequiredService<PluginManager>();
        EventBus = provider.GetRequiredService<IMinecraftEventBus>();
        _config = ServerConfiguration.Default;
    }

    public static Version McVersion => new(Assembly
        .GetExecutingAssembly()
        .GetCustomAttribute<MinecraftVersionAttribute>()?
        .MinecraftVersion.ToString(3) ?? string.Empty);

    public static Version? Version => Assembly
        .GetExecutingAssembly()
        .GetName()
        .Version;

    public IMinecraftEventBus EventBus { get; }


    public ValueTask<ServerStatus> GetStatus()
    {
        return ValueTask.FromResult(new ServerStatus
        {
            Version = new ServerVersionModel
            {
                Name = "1.18.1",
                Protocol = 757
            },
            Players = new OnlinePlayersModel
            {
                Max = 9,
                Online = 6,
                Sample = new[]
                {
                    new OnlinePlayerModel
                    {
                        Name = "Notch",
                        Id = new Guid().ToString()
                    }
                }
            },
            Description = new DescriptionModel
            {
                Text = "A server running on Beacon!"
            },
            Favicon = ServerIcon
        });
    }

    protected override async Task ExecuteAsync(CancellationToken cToken)
    {
#if DEBUG
        _logger.LogWarning("Running in debug mode! Performance will be impaired");
#else
        _logger.LogInformation("Running in production mode");
#endif
        _logger.LogDebug("Debug logs are enabled for this log sink");

        await LoadConfigurationAsync();

        var watch = Stopwatch.StartNew();
        await InitializeAsync();
        watch.Stop();

        _logger.LogInformation("Server booted in {Seconds} seconds", watch.ElapsedMilliseconds / 1000.0);

        await Task.WhenAny(
            AcceptConnectionsAsync(cToken),
            AcceptCommandLineInput(cToken)
        );
    }

    private async Task InitializeAsync()
    {
        await _pluginManager.LoadPlugins();
    }

    private async Task LoadConfigurationAsync()
    {
        _logger.LogInformation("Reading configuration");
        var file = new FileInfo("server_settings.json");

        if (!file.Exists)
        {
            await using var stream = file.OpenWrite();
            await JsonSerializer.SerializeAsync(stream, _config);
            return;
        }

        try
        {
            await using var stream = file.OpenRead();
            var parsedConfig = await JsonSerializer.DeserializeAsync<ServerConfiguration>(stream);
            if (parsedConfig != null)
            {
                _config = parsedConfig;
                return;
            }

            _logger.LogWarning("Could not read server configuration. Resetting to the default one");
        }
        catch (Exception e)
        {
            _logger.LogWarning(e, "Could not read server configuration. Resetting to the default one");
        }

        file.Delete();
        await JsonSerializer.SerializeAsync(file.OpenWrite(), _config);
    }

    private Task AcceptCommandLineInput(CancellationToken cToken)
    {
        var sender = new ConsoleSender(this);
        var commands = new List<BeaconCommand>
        {
            new ServerCommand(this)
        };
        // commands.AddRange(_pluginController.GetRegisteredCommands());

        var commandHandler = new CommandHandler(_logger, commands);
        var autoCompleteTree = commandHandler.GetAutoCompleteTree();
        var consoleHandler = new ConsoleHandler(autoCompleteTree);
        consoleHandler.EnteredCommand += async (_, cmd) => await commandHandler.HandleAsync(sender, cmd, cToken);
        return consoleHandler.Handle(cToken);
    }

    private async Task AcceptConnectionsAsync(CancellationToken cToken)
    {
        _logger.LogInformation("Start listening for clients");

        var listener = new TcpListener(IPAddress.Any, 25565);
        listener.Start();

        while (!cToken.IsCancellationRequested)
        {
            var client = await listener.AcceptTcpClientAsync(cToken);
            _ = HandleTcpClient(client, cToken);
        }
    }

    private async Task HandleTcpClient(TcpClient tcp, CancellationToken cToken)
    {
        var connection = new BeaconConnection(this, tcp);

        // Fire event to notify plugins a TCP connection has been made.
        var e = new TcpConnectedEvent(this, connection);
        await EventBus.FireEventAsync(e, cToken);

        if (e.IsCancelled)
        {
            connection.Dispose();
            return;
        }

        var faulted = false;

        try
        {
            await connection.AcceptPacketsAsync(cToken);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An exception occured while a connection was accepting packets");
            faulted = true;
        }
        finally
        {
            connection.Dispose();
        }

        // Fire event to notify plugins a TCP connection has been lost.
        await EventBus.FireEventAsync(new TcpDisconnectedEvent(this, faulted, connection.RemoteAddress), cToken);
    }
}*/

